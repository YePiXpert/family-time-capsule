...
- PDFKit 使用许可字体及固定块模板，不执行用户 HTML，不请求用户 URL、在线字体或脚本。
  独立进程只接收服务器解析的原件路径；图片从原件生成有界衍生图，不覆盖来源。
- 入队、渲染前、渲染后、原子完成前及每次下载重验当前主体与来源摘要；来源删除/变化或
  权限收紧后旧产物返回拒绝，不能借旧 URL 绕过。家庭版继续按家庭读者过滤全部嵌套来源。
- 队列总量 100、每家庭 20，全局渲染并发 1，180 秒超时，200 页/256 MiB 单产物、
  2 GiB 每家庭出版缓存；Node 堆 384 MiB 不是 OS RSS 硬限额。临时目录只存受限 JSON/输出，
  失败清理，产物原子 rename 后发布；中断任务租约到期可重试。
...
账号级失败计数与锁定。原生已有账号登录、建家庭页的已有账号入口、设置页连接入口
均支持动态码与恢复码第二腿。App 使用 `x-ftc-native-auth: 1`，服务端只把认证库签名的
临时 two_factor cookie 转成专用响应头；App 只在当前表单内存中保存该凭据，验证完成
之前不调用账号连接/同步，也不写 Keychain。原生传输忽略已有 cookie、Bearer 和可信
设备 cookie，第二腿仅使用显式待验证凭据；不影响普通浏览器 cookie 登录。
验证码、恢复码、签名、过期、单次消费、HTTP 限流与账号锁定仍由认证库处理。
取消验证清空本机临时凭据；离开页面后迟到的成功响应不会连接账号，并尽力撤销该会话。
旧服务端不返回原生验证凭据时提示升级，不能静默跳过两步验证。
当前证据为真实 SQLite/HTTP 和原生控件平台替身；签名包/真机仍待验收。
两步验证启用/关闭/恢复码管理以及 Passkey 仍在 Web 完成，未声称完整原生安全管理已完成。
...
## 19. 访客限定阅读与孩子账号（M2-d，migration 0050）

**访客限定阅读链接（ID-5）**：与投递箱（只提交）严格分立的只读 scope。
管理员可为「单一相册」生成只读链接：令牌 256-bit 只存 SHA-256（0050 表
guest_read_grant），可设 7/30/90 天或长期，可随时收回（即时失效）；浏览
计数与最近访问留痕。`/view/[token]` 不暴露家庭名、成员或其他相册；
`/api/media` 的访客分支只放行「授权相册内记忆事件的资产」，范围外一律
401/404，且强制 inline 展示（不提供附件下载语义）。

**孩子本人账号（ID-16/SEC-5）**：绑定经邀请进行，但只允许 viewer/
contributor 角色，且发起者必须是本家庭在册监护人（服务内二次校验），
审计 `person.child_account_invited` 记录谁为孩子开通、以何角色。账号
日后的角色变更同样封死 admin/editor 提权路径。绑定不解锁
child_later 内容——解锁仍只能由监护人手工逐孩触发且不可撤销。
