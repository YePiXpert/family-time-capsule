...
> 当前开发版本：`1.0.0-dev.1`；Expo 展示版本为 `1.0.0`，iOS buildNumber / Android
> versionCode 为 `13`。手机是主要使用场景；每次交付必须提供同一 `main` 提交的 APK 与自签用 IPA。
...
记忆详情的服务器缓存按服务器、会话摘要、账号和家庭隔离；切换连接时不会沿用旧详情，
401/403/404 会撤下对应缓存。升级时清除无法证明归属的旧详情缓存，重新登录后需联网
打开一次以建立当前会话的缓存；本机采集原件和待同步记录保留。
...
这些写入需要联网；冲突保留本次输入。持久年册与主动离线收藏见下文 1.2 章节。

### 1.2 媒体阅读（开发切片）

记忆图片、音视频与人物最近声音接入 Modal 阅读器；前后切换、页码、原图按需加载、缩放
按钮与 iOS ScrollView 缩放、主动连续音频、进度/倍速/秒数定位、真实转录点句、来源跳转。
只在打开当前音视频时创建 expo-audio/expo-video player；翻页和关闭卸载，按 SDK 57 hook
管理释放。本机原件也可通过同一阅读器打开，不为所有离线素材同时创建播放器。

expo-sharing SDK 57 用于用户主动导出原件副本；下载使用 expo-file-system/legacy。
复杂系统导出、Android/iOS 解码/方向/触控/安全区验收不由组件测试或 Expo export 替代。

### 1.2 原生书架与编辑

更多 → 家庭书架 → 作品：真实服务器选材（记忆/相册/已发布故事）、三模板、读者范围、
标题/副标题/日期/封面、章节与内容排序、正文/说明、版式、照片选择、快照及删除恢复。
阅读按章节、每段最多 8 块加载，照片先用预览衍生物；来源跳回原记忆。自动保存保留请求
期间的新输入，失败/冲突暂停并保留输入，离页可保存或继续编辑。精细焦点与整册排版仍用 Web。
真实组件交互使用测试 host，不能替代真机触控、安全区、系统分享和旧包升级验收。
出版与主动离线收藏均已接入，见下文。

### 1.2 原生出版

BookDetail 实际接入保存后生成 PDF/EPUB、队列进度、取消、重试和系统副本导出；生成期间
服务器仍可记录。导出前显示容量、版本及家庭/私人范围说明，下载再次校验权限，失败或
字节不完整不进入系统导出，并只清理本次 `cacheDirectory/publication-*` 文件。
系统导出与主动离线收藏分别有明确入口；后者的暂停/继续、离线书架与撤权缓存见下文。
原生出版组件/文件导出交互新增 2 项测试；系统导出 sheet 和实际阅读器安装仍需真机验收。

### 1.2 回顾入口

首页本月回顾、书架进入 BookReview；可调整月/年或 1–366 天日期，选择出生第一周/首月/百天，
浏览真实素材、人工精选、勾选建立相册或年册，继续旧稿及明确追加。BookDetail 可复制新册、
标记完成/重开；复杂焦点编辑仍使用 Web。首页显示最多三份实际 active 作品，无贡献排行榜。

## 1.2 主动离线收藏

相册/作品详情的“下载供离线阅读”先保存当前编辑并显示实际清单容量、照片/音视频/文档数量。
更多 → 离线收藏提供阅读、暂停、继续、失败重试、单份/当前连接清理；保留五个一级标签。
阅读器按章节每组 8 个块展示，图片完整比例并可全屏缩放，音视频仅打开当前播放器；
本用户章节/阅读组/播放秒数持久保存。精细焦点裁切仍以 Web/PDF 为准。

`family-reader-cache.sqlite` 和 `documentDirectory/reader-downloads/` 专用于服务器阅读副本，
与 captures、intake 和 outbox 物理分离。键包含 server/account/family；离线账号匹配使用
凭据摘要，不在缓存中保存 token、下载授权头或原生 resume blob。单连接 512 MiB、全机
1 GiB/100 份，预留 64 MiB 可用磁盘。一次仅下载一份；暂停保留已完整校验的文件，
未完成文件继续时重新下载。SHA 采用 64 KiB 增量读取，不将整段视频载入 JS 内存。

每次下载前后、重新阅读及首页联网刷新重新验证账号、作品版本和来源范围。401/403/404/409
撤下副本与当前阅读页；断网允许上次成功副本继续阅读，503 不被解释为断网或权限拒绝。
离线时无法立即远程撤权。配额/进度/下载任务属于设备，不进入家庭 portable archive。
