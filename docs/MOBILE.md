...
> 当前候选版本：`1.0.0-rc.3`；Expo 商店展示版本保持 `1.0.0`，原生 iOS build / Android
> versionCode 为 `3`，以便从上一候选包升级安装。
...
移动端自动化现有 4 个测试文件 / 14 个场景，覆盖本机数据库启动、五项导航与页面拆分、
离线文字和媒体原件保全、直接录音/相册入口、分页整理、断网重启、恢复网络后的幂等补传，
以及单条失败不阻塞后续同步。相机、相册选择器、真实音频焦点与系统杀进程后的文件行为仍须
按 `REAL_DEVICE_TEST.md` 在 iOS/Android 硬件上留档。
