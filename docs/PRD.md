...
材质语言为暖色液态玻璃：导航、卡片、弹层、控件使用半透明玻璃（分级见
`design-system/family-time-capsule/MASTER.md`），正文与成长册纸张保持实底。
交互对齐 iOS：大标题页头、底部弹层、滑动操作、长按菜单与触觉反馈。
...
月度/年度回顾及自选 1–366 天范围复用人工精选，一键生成可编辑草稿；重复点击继续原稿，
新记忆只提示可加入，复制新册需明确操作。首页显示本月回顾及真实正在制作作品。

1.2 M7 已落地：详情主动下载与独立阅读缓存，清单容量、每连接/全机配额、暂停后续传已完成
文件、校验失败重试、当前用户阅读/播放进度。ZIP 经实际离线 file:// 浏览器读取。
权限拒绝撤回服务器副本，断网保留最近校验内容；清理不触碰本机原件/outbox。
