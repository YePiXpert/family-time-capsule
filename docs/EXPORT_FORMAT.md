...
- 当前 `exportVersion: 3`，应用版本仍为 `1.0.0-dev.1`。归档协议版本与应用发布版本分开。
- v3 在 v2 的权限和正文格式上，要求每条故事携带 `inputSources`（完整模型输入依赖数组，或 `null` 表示历史来源未能证实）。依赖独立于段落，删除段落不会抹去标题或其他段落的撤权依据。旧 v2 恢复器拒绝版本 3，避免静默丢弃这些依赖。
- v2/v3 必须包含 `privacy.json` 和每条记忆的 `bodyText`；缺失或权限引用不完整时拒绝恢复。旧恢复器会拒绝版本 2，避免将私密内容按 v1 的家庭共享默认值读入。
- 当前恢复器继续读取真实 v1/v2 归档；旧故事缺少 `inputSources` 时保留标题、正文及原有引用，按来源未证实暂停在线展示和作品使用，不自动按空依赖公开。可基于当前允许家庭阅读的素材重新组装故事；旧稿仍保全在灾难档案中。
- 真实 v1 归档：v1 无法证明原事件的完整账号授权：发现私密/指定成员草稿时，将对应事件收紧为私密并创建待确认身份；没有保存过的正文或授权不能凭空补回。
...
BookRenderJob `reading_zip` 与 PDF/EPUB 使用相同版本、目标读者、并发/取消/原子完成和
实时下载校验；幂等摘要额外包含随册声音/视频/文档来源与权限。ZIP 根包含 `index.html`、
`style.css`、`sources.txt`、`阅读说明.txt`，以及本地 `images/`、`media/`。
解压后直接 file:// 阅读，无登录、网络、脚本、fetch 本地 JSON 或在线字体。
图片从原件生成阅读衍生图；允许的附带音视频/文档流式写入且字节不变。

这是**精选阅读包**，不是可恢复的完整备份。播放取决于接收设备编解码器；提供保存媒体
副本入口。包不携带内部 API、凭据；下载副本可以被保存/转发，无法远程收回。

### v1.4 名称审核记录

新增可选 `name-reviews.json`，由 `manifest.modules.nameReviews = 1` 声明。
保存经过版本化人工操作的标题采用、忽略、撤销记录，包括目标、revision、名称
来源、采用前名称及原采用版本。当前未采用的 AI 输出不进入此文件。
声明模块但缺文件、重复记录、未知目标和矛盾版本会在恢复写入前被拒绝。

原件名称/字节/路径/哈希与名称编辑分开保存。Provider Key、consent、任务、
