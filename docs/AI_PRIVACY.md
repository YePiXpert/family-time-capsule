...
> 当前状态：**安全启用基础设施已实现，真实素材处理器已扩展至图片视觉分析与事件元数据建议**。仓库已有
> provider-neutral 适配器、SQLite job queue、独立 worker、`/settings/ai`
> 披露与逐能力同意。production handler registry 已注册 `transcribe.asset.v1`
>（音频/视频转录）、`analyze.asset_image.v1`（图片视觉分析）与 `suggest.event_metadata.v1`
>（事件标题/地点/参与人/标签建议），只有具备 `ai:review` 能力的用户才能逐项触发；
> 即使配置 `AI_PROVIDER` 并完成同意，系统仍不会自动扫描或发送其他家庭资料。
...
- 具备 `ai:review` 的用户可在记忆详情页为音频/视频原件请求 AI 转录，或为图片原件请求 AI 视觉分析，或为事件请求 AI 元数据建议；
...
- 事件元数据建议产出 `ai_suggestion` 与 `ai_suggested` fact（带 `fact_source`），必须由用户逐项接受/拒绝，不会自动确认；
...
- 执行 embedding job；该 handler 将在对应业务切片实现后注册；
- AI 自动确认 Fact 或覆盖用户修订的转录文本；
- 在 AI 建议上下文中发送 private Contribution 或跨家庭资料；
- 在 SQLite 保存 API key；
...
| 用户确认 Fact | 耐久事实 | 必须带来源（`fact_source`）导出/恢复，AI 不得直接创建 |
| FactSource | 耐久来源追踪 | 必须随家庭 archive 导出/恢复 |
| MemoryEventTag | 耐久家庭资料 | 必须随家庭 archive 导出/恢复 |
| `ai_suggestion` | 运维/可重建状态 | 不进入 portable archive；恢复后需重新请求/接受 |
...
## 搜索与供应商存储边界（2026-09-08）

自然语言检索只发送用户本次输入的句子，不发送整个家庭索引、人物目录或命中
正文。模型没有工具权限，不能执行 SQL；非法或无法解析的限制不会被静默丢弃。
转换结果与当前账号及同意版本绑定，撤回后重新启用也不会复活旧结果。
`ai_search_operation` 是短期、按账号隔离的派生数据；`ai_dispatch` 只保留操作
身份、数量与发送状态，不存请求正文或 Key。二者均不作为家庭归档的事实来源。

Responses 请求的 `store: false` 请求供应商不要保留可检索的响应状态。
[OpenAI 数据控制文档](https://developers.openai.com/api/docs/guides/your-data)
仍区分应用状态与滥用监控等留存；该参数不代表零留存、端到端加密或第三方 CPA
也遵守相同政策。自有或第三方接收服务按实际设置披露，Key 只留服务端。
