...
- 第一轮根全量因并行 mobile npm ci 暂时移除了共享模块依赖的 expo/tsconfig.base 而失败；完成安装后重新完整验证通过，未修改断言或平台跳过条件。本批测试都在隔离数据目录；日志 /tmp/ftc-native-auth-*.log。后续同 SHA CI 仍须检查；没有自动更新 root 运行实例。
