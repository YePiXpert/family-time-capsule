...
## D-023（1.2 日历）日历周年与批次读取

- 家庭日/月/年筛选统一为本地日界换算后的 UTC 半开区间；百天是日历天差，满月是
  日历月周年。月末周年向目标月份末日收敛（1/31 → 2/28 或 2/29），不再以 30 天冒充满月。
- 日历每天的边界构成一个 SQLite CTE：一次聚合计数、一次窗口函数封面查询（每天最多 3 张），
  不逐天查询。媒体筛选与封面在查询内应用原件/衍生物树的实时讲述权限。
- 时间轴保留稳定 `(occurredAt,id)` cursor；新日历同样使用二元 keyset。年份筛选词汇由
