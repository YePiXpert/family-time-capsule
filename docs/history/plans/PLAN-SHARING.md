...
> **历史记录**：2026-09-21 批准，交付为 Build 72。用恢复码加入的方式与备份页的「家人一起写」卡，先后被 1.1.0 的设备授权和 1.1.4 家庭页的同步卡取代。正文保留当时的状态、命令与待办，不代表现在。当前范围见 [PRODUCT](../../../PRODUCT.md)，交付状态见 [HANDOFF](../../../HANDOFF.md)，全部历史文档见 [索引](../README.md)。
...
5. **合并规则**（`merge.ts` 纯函数，逐实体）：
   ...
   **实施修订（提交 13b）**：两台在旧基上各改一版、轮流发布时，先发布的一方已把自己的版本记成合并之基，后来接收赢家会被当成普通更新，导致输的一方手机没有留底。records 与 letters 新增可选 `ancestors?: string[]`，记录直接与间接源版本的内容哈希前 16 位（小写十六进制），最新在前、最多 8 枚；内容哈希排除 `revision`、`updatedAt` 与 `ancestors`。保存、补落款、改信、封存、拆封与换回留底均延续世系，接收远端版保留对方世系。「本机没动」且接收内容不同的远端版时，用 `descendsFrom` 三态判断：有世系且含本机哈希为 `true`（顺序编辑，不出卡），有世系但不含为 `false`（并发编辑，接收赢家并给本机输掉的一版留底，`device: null`），未带数组为 `undefined`（旧版未知，不出卡，避免升级期误报）；只为记录与信留底，两边都动的分支不变。
...
6. **一次同步**（`family.ts` 的 `runFamilySync`）：核对钥匙 → `GET /backup/manifests` → 跳过 sha 没变的设备（`sync/state.json` 的 `seen`）→ 逐份下载解密清单 → `merge` → 先把缺的素材逐个下载进 blob 库（写一枚 `restoring-*.xmbm.part` 钉子保护，可停可续）→ 物化到 media 目录并生成缩略图 → **一次** `store.change` 写入合并结果 → `createBackup` 出新清单 → 只传缺的对象 → `PUT /backup/manifest`（服务端按设备存）→ `prune` → 写 base／seen／state。任何一步失败，本机库一个字节没动（与 `restoreBackup` 同一哲学）。
   **实施修订（提交 10）**：一次写库前在写队列内用最新本机资料重跑合并，文件准备失败只清本轮新建文件，已下载 blob 由清单钉子保护。写库成功后先保存冲突留底，再推送；上传失败保留已经合入的资料与冲突，下一次重试。base／seen 仍在推送成功后更新。本机设备身份取自 `GET /api/v1/me` 的 `member.deviceId`，不靠 `GET /backup/manifest` 的成员回退；同步成功将本机 ID 记入 state，退出优先用它、未记录则查询 `/me`，只删本机清单。本机从未发布而返回 `NOT_FOUND` 时仍清钥匙与三个同步文件，库和素材保留。「失败本机不变」的边界是写库提交之前，提交之后不回滚主人可能继续写下的内容。
