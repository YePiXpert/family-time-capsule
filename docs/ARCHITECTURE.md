...
- **合并**（`sync/merge.ts`，纯函数）：读别的设备的清单，按实体做三方合并。
  ...
  - 根值带版本：资料各字段、每年的寄语与封面各有一个改动时刻（`Library.rootStamps`），人物有 `updatedAt`。`LocalStore.change` 在值变了时盖上「现在」（至少晚于上一版一毫秒），合并与恢复自己写时刻。带时刻的版本按（时刻，内容指纹）全序比较，取最新的一版，所以改回见过的值、清空都传得开，旧快照、没有时刻的旧备份传不开，几台手机不会来回翻。两台同时改同一年的寄语照旧接起来，接好的一段时刻更晚，全家收敛到它。没有时刻的版本（1.1.6 手机、升级前的数据）按原来的规则（`known`、内容哈希）并，并进来就盖上时刻；1.1.6 手机忽略 `rootStamps`，跟不上根值的改回。
  - 头一回加入：用一把新钥匙加入（`joinFamily` 清掉同步文件时）会在 `state.json` 记下 `joining`，直到第一轮合并落进本机库才去掉。这期间家里清单里有的根值、年度目录与同 id 的人物听家里的（家里带时刻的最新一版；都没时刻就在家里的几版里按原规则挑），加入前在设置里填的值时刻再新也不算数；家里没有的由本机补上，带着时刻传开。同一年的寄语两边都有就接起来。建家的那台读不到别人的清单，什么都不让。恢复备份、合并历史丢失都不算加入。
