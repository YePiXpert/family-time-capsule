...
上述里程碑表记录既有交付与测试范围，不代表所有中断、恢复及分页场景已经验收完成。
`v1.1.0-alpha.1` 已发布于 `c885217`；main 后续修复独立提交，已发布标签保持原位。

- `86e8e6e`：将完成事务后的临时清理移出原件回滚范围；并发创建和重启上传在 SQLite
...
来源、默认时间地点标题或参与人不一致返回 409。原生媒体 outbox 从本机关系表推导批次 ID，
上传前幂等创建会话并把 ID 交给续传 API。旧的未分组原生媒体上传在声明、创建者和来源一致时
可补关联，保留 offset、原件及完成结果；补关联与 complete 共用上传锁。回归覆盖导出中的
批次/Asset/Inbox 关系。纯文字与 URL 分享也先幂等创建批次，再原子提交 Inbox 和批次项；
