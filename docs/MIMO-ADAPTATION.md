...
主人随后明确选择 `mimo-v2.6-pro`、`https://token-plan-cn.xiaomimimo.com/v1` 并提供套餐密钥。密钥仅保存于服务端权限 0600 的文件，两路显式配置并独立只读挂载。`token-plan-authorized` 记录这次主人的使用选择，不代表小米特别许可；官方 Token Plan 编程工具使用限制仍存在。真实验证和生产切换结果将在完成后补充。
