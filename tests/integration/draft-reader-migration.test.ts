...
it("upgrades populated 0060 drafts without losing items, intake references, values or foreign keys", () => {
  ...
  try {
    ...
    for (const table of Object.keys(before)) expect(db.prepare(`select * from ${table}`).all()).toEqual(
      table === "draft_item" ? (before[table] as object[]).map(row => ({ ...row, live_photo_group_id: null, live_photo_role: null })) : table === "draft" ? (before[table] as object[]).map(row => ({ ...row, reviewed_revision: null, purpose: "capture", edit_target_memory_id: null, milestone_type: null })) : before[table],
    );
    expect((db.pragma("foreign_key_list(draft)") as {from:string}[]).filter(key => key.from !== "edit_target_memory_id").map(({...key}) => ({...key, id: 0}))).toEqual((foreignKeys as object[]).map(key => ({...key, id: 0})));
