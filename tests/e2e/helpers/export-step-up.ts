...
/**
 * ID-10 step-up：完整导出要求 10 分钟内的登录或密码复核。
 * 定位必须限定在「完整导出」面板内——设置页的删除账号区域同样有
 * 「当前密码」输入框，裸定位会错抓（CI run 34056721388 的失败原因）。
 */
export async function grantExportStepUp(page: Page) {
  await page.goto("/settings");
  const panel = page.getByRole("region", { name: "完整导出" });
  await expect(panel).toBeVisible({ timeout: 20_000 });
  const link = panel.getByRole("link", { name: "导出完整备份 ZIP" });
