...
export async function createReadGrant(
  ...
  const saved = db.transaction((tx) => {
    if (!tx.get(sql`select id from user where id=${context.userId} and family_id=${context.familyId}
      and role=${context.role} and role in ('owner','admin') and disabled_at is null`)) return false;
...
/** Every use rechecks the collection and the issuer, including cached resolutions. */
function activeGrantPredicate(grant: Pick<ResolvedReadGrant, "grantId" | "familyId" | "collectionId">): SQL {
  return sql`exists(select 1 from guest_read_grant g
    join collection col on col.id=g.collection_id and col.family_id=g.family_id
    join user issuer on issuer.id=g.created_by_user_id and issuer.family_id=g.family_id
    where g.id=${grant.grantId} and g.family_id=${grant.familyId} and g.collection_id=${grant.collectionId}
      and g.revoked_at is null and (g.expires_at is null or g.expires_at>unixepoch())
      and col.deleted_at is null and issuer.disabled_at is null and issuer.role in ('owner','admin'))`;
}
...
export function resolveReadGrant(token: string): ResolvedReadGrant | null {
  ...
  const row = db
    ...
    .all()[0];
  if (!row) return null;
  if (!db.get(sql`select 1 where ${activeGrantPredicate(row)}`)) return null;
  const now = new Date();
...
export function listReadGrantEntries(grant: ResolvedReadGrant): ReadGrantEntryDto[] {
  const rows = getDb().all<{ id: string; eventId: string | null; assetId: string | null; title: string; caption: string }>(sql`
    ...
    from collection_item ci left join memory_event e on e.id=ci.memory_event_id and e.family_id=${grant.familyId} and e.deleted_at is null and e.status='confirmed' and e.visibility='family'
    ...
    where ci.collection_id=${grant.collectionId} and ci.family_id=${grant.familyId} and (e.id is not null or a.id is not null)
      and ${activeGrantPredicate(grant)} order by ci.position`);
...
export function readGrantIncludesAsset(grant: ResolvedReadGrant, assetId: string): boolean {
  ...
  return Boolean(getDb().get(sql`
    ...
    select 1 from root where
      ${activeGrantPredicate(grant)}
      and exists(select 1 from collection_item ci left join memory_event e on e.id=ci.memory_event_id and e.family_id=${grant.familyId} and e.deleted_at is null and e.status='confirmed' and e.visibility='family'
        where ci.collection_id=${grant.collectionId} and ci.family_id=${grant.familyId} and (ci.asset_id in (select id from tree) or (e.id is not null and exists(select 1 from memory_event_asset ma where ma.memory_event_id=e.id and ma.family_id=${grant.familyId} and ma.asset_id in (select id from tree)))))
      and (
        exists(select 1 from asset a where a.id=root.id and a.visibility='family')
        or exists(select 1 from tree shared where ${familyReviewAssetPredicate(grant.familyId, sql`shared.id`)})
        or exists(select 1 from memory_event e join memory_event_asset ma on ma.memory_event_id=e.id and ma.family_id=e.family_id
          where e.family_id=${grant.familyId} and e.visibility='family' and e.status='confirmed' and e.deleted_at is null and ma.asset_id in (select id from tree))
      )
