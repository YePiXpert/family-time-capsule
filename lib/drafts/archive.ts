...
export type DraftArchive = DraftContent & { id: string; inboxItemId: string | null; authorPersonId: string | null; authorName: string; status: "editing" | "published" | "discarded"; memoryEventId: string | null; createdAt: string; updatedAt: string };
...
export function parseDraftArchive(value: unknown, refs: { assets: Set<string>; events: Set<string>; people: Set<string>; inbox: Set<string> }): DraftArchive[] {
  ...
  return value.map(raw => {
    ...
    return { ...content, id, inboxItemId: raw.inboxItemId, authorPersonId: raw.authorPersonId, authorName: raw.authorName, status: raw.status, memoryEventId: raw.memoryEventId, createdAt: raw.createdAt, updatedAt: raw.updatedAt };
  });
}
