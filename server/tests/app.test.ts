...
function fixture(provider?:Provider) {
 ...
 const input=()=>({requestId:randomUUID(),model:'mimo-v2.6-flash',photos:[{id:'a',date:'2020-01-01T12:00:00',image}]});
...
test('server restart cannot repeat an uncertain paid request',async()=>{
 ...
 f.store.reserve(f.member.member,id,'hash',1,0,'mimo-v2.6-flash');f.store.recover();
 assert.equal(f.store.reserve(f.member.member,id,'hash',1,0,'mimo-v2.6-flash'),'failed');
 await f.app.close();f.store.close();
});
...
test('fixed MiMo configuration preserves quotas and normalizes previous app selections',async()=>{
 ...
 assert.equal(config.defaultModel,'mimo-v2.6-flash');assert.equal(config.reasoningEffort,'per-mode');
 assert.deepEqual(config.models,[{id:'mimo-v2.6-flash',label:'MiMo 2.6 Flash'}]);
 ...
 assert.equal(update.statusCode,200);assert.deepEqual(f.store.settings().enabledModels,['mimo-v2.6-flash']);
 assert.equal(f.store.settings().globalPhotos,90);
 for(const model of ['mimo-v2.5','deepseek-flash','gpt-5.6-luna',undefined]){
  const response=await f.app.inject({method:'POST',url:'/api/v1/ai/write',headers:f.headers(),payload:{...f.input(),model}});
  assert.equal(response.statusCode,200);assert.equal(actualModel,'mimo-v2.6-flash');assert.equal(response.json().model,'mimo-v2.6-flash');
 }
 await f.app.close();f.store.close();
});
...
test('polish carries only the stored text with an explicit length ceiling',async()=>{
 ...
 const polish=()=>({requestId:randomUUID(),model:'mimo-v2.6-flash',photos:[],mode:'photos',writingMode:'polish' as const,context:'标题：原稿\n正文：\n我们一起去公园。'});
