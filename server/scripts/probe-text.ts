...
// One synthetic example per mode: five calls maximum, no retry or output text.
const seen=new Set<WritingMode>();
for(const {mode,context} of samples){
 if(seen.has(mode))continue;seen.add(mode);
 await probe({requestId:randomUUID(),model:MODEL_ID,writingMode:mode,photos:[],context});
}
