#!/usr/bin/env python3
"""探反代的请求体上限：登录 → 往备份对象端点 PUT 若干 MB → 看应答是我们的 JSON 还是反代的拦截页 → 删掉 → 撤销探测设备。

期望：4 MB → 201（我们收下了）；9 MB → 我们的 JSON 413（code TOO_LARGE）。
若 4 MB 就被非 JSON 的 413 拦下，是反代（nginx 默认 1 MB）在拦：加 client_max_body_size 16m; proxy_request_buffering off; proxy_read_timeout 130s; 后重探。
"""
import argparse,hashlib,json,os,subprocess,sys,urllib.error,urllib.request
p=argparse.ArgumentParser();p.add_argument('--base',default='https://capsule.yep.li/api/v1');p.add_argument('--username',required=True);p.add_argument('--password',required=True)
...
status,body,_=request('/login',json.dumps({'username':args.username,'password':args.password,'deviceName':'upload-limit-probe'}).encode(),headers={'Content-Type':'application/json'})
...
try:
 for mb in args.mb:
  ...
  if mb<=8 and status not in (200,201):ok=False
  if mb>8 and not (ours and status==413):ok=False
finally:
 request('/backup',None,token,'DELETE')
 if args.container:
