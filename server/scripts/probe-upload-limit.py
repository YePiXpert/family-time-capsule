...
带 --authenticated --container：在容器里给第一位管理者临时登记一台探测设备后真上传，期望 4 MB → 201（我们收下了）、9 MB → 我们的 JSON 413（TOO_LARGE），结束只撤销探测设备；无清单引用的探测对象留给服务端回收。
...
if args.authenticated:
 if not args.container:p.error('--authenticated needs --container')
 # 没有密码了：直接在容器里给第一位管理者登记一台探测设备，令牌只进本进程、不打印；结束时撤销。
 script="import{Store}from'./src/store.ts';const s=new Store('/data/ai.sqlite');process.stdout.write(JSON.stringify(s.attach(s.admins()[0].id,'upload-limit-probe')));s.close();"
 login=json.loads(subprocess.check_output(['docker','exec',args.container,'node','--input-type=module','-e',script],text=True));token=login['token']
ok=True
try:
 for mb in args.mb:
