...
> 2026-09-21 定位重述：新增 `PRODUCT.md`（不是相册，是一家人写给她的传家册；AI 从代笔改为访谈者与整理者），Build 72 改为「家人一起写」（落款先于同步），原 73「分享」不再单列——见第四节。
...
- **下一步**：Build 72「家人一起写」——落款先于同步（第四节，次序以 `PRODUCT.md` 第八节为准，待主人拍板）。改 `Library` 前先出 `docs/plans/PLAN-SHARING.md`，落款与同步写进同一份计划。
...
第三步·Build 72「家人一起写」（PRODUCT.md 第八节次序 1；主人若另有拍板以主人为准）：
- 落款先于同步：每段时光有「谁写的」（这台手机的默认落款 + 每段可改；信的 from 已是先例），
...
- 之后按 PRODUCT.md 第八节：说一段（本机转写）→ 出生的故事 → 访谈者（AI 追问）→ 家史 → 年度册的编者。原 73「分享」不再单列。
```
...
- **说一段**：编辑页录音 → 本机转写成正文，录音留在这段时光里；先试系统自带的本机识别，声音默认不出手机。
- **出生的故事**：一组带着人写的长故事（出生那天、怀孕、名字、我们怎么认识的），进第一本纸书的第一章。
- **访谈者**：AI 追问（新的 writingMode，只回问题，不写正文）；今天的小问题带上下文；写信引导。
- **家史**：给外公外婆爷爷奶奶的访谈与口述。
