...
> 最后更新：2026-09-24。**1.1.2（构建号 84）已完成构建与产物核验并交付**（第一节第一条）。两台真机验收仍待完成；服务端切换记录见第一节。
>
...
- **1.1.2 安装包（构建号 84，2026-09-24 已交付）**：发布源码 `7e4a8a0788ada4b8daa4552db49d6f308f598a81`，轻量标签 `v1.1.2`，[run 36015813965](https://github.com/YePiXpert/family-time-capsule/actions/runs/36015813965) 八个作业全部成功，未另派重复构建。修复范围见 CHANGELOG「1.1.2」，原有 11 个未提交文件未进入本版；家庭初始化分步界面仍在这些未提交修改中。
  - 发布快照门禁：手机端 825 项、服务端 204 项测试，双端类型检查、手机端 Lint 与本机边界检查通过。
  - [Release v1.1.2](https://github.com/YePiXpert/family-time-capsule/releases/tag/v1.1.2) 的 APK、未签名设备 IPA、build-source.json 与 sha256sums.txt 已下载核验。三项 SHA-256 全部匹配；源码记录完整 SHA 一致；APK 清单、IPA 主应用和分享扩展均为 1.1.2／84；Release 文件与构建产物逐字节一致。
  - 三份验证报告的 gitSha 都等于发布源码、success 为 true：安卓 14 项布尔检查全部通过；iOS 启动 9 项通过；iOS 完整回归的纸书 PDF、开放归档、记录身份、专题册重启、跨月选材、完整备份恢复、损坏库恢复和原库保留均通过。下载与核验摘要在 `/var/tmp/anan-1.1.2-verified/verification.json`。以上是模拟器与 CI 证据，**两台真机验收仍待完成**。
  - 等待规则已按主人更正：只有 Fable 默认不轮询，其他所有模型（含未知模型）默认每 30–60 秒跟踪到完成并验收交付；详见 AGENTS.md。
  - 本版手机同步与取消修复不依赖服务端更新；服务端最新部署见下条。


- **服务端 GPT-6 Astra medium（2026-09-24 15:21 UTC 已部署）**：主人指定新兼容上游与密钥，最终文字模型为 `gpt-6-astra`、五种模式统一 `medium`，专用转写为 `mimo-v2.5-asr`；随后明确「没有历史包袱」，已删除旧模型名称的兼容列表，显式传其他模型一律 400。手机请求不携带模型名，使用服务端固定模型。提交 `9f6d252` 与 `5eee855`，生产完整源码 **`5eee8556c95b72b063b1381b9fc43a6b0b59acc2`**，从 `bc948a1` 切换。
  - 当前只使用私有 `UPSTREAM_BASE_URL`／`UPSTREAM_KEY_PATH`，两类请求共用一个密钥挂载；原 DeepSeek／MiMo 专用提供商、地址、密钥和 Token Plan 字段已从活动配置移除，不读旧变量、不回退。旧供应商密钥已移出活动目录，仅在受限的部署回滚快照中保留；仓库、日志、本文均不含实际地址或密钥。
  - 文字请求使用 `reasoning_effort: medium`、`max_completion_tokens: 16384`、非流式 JSON，原五条提示词与业务校验保持。独立提交快照门禁：手机端 825 项、服务端 203 项测试，双端类型检查、手机端 Lint 与本机边界通过（服务端配置测试去掉旧 Token Plan 路径，因此比 204 少一项）。原有 11 个未提交文件保持原样，未打包、未部署。
  - 最终镜像隔离 staging：五种文字任务均真实调用成功（约 4–14 秒），`mimo-v2.5-asr` 合成音频转写成功；重复请求不重复计费／扣额、无效任务、上传与转写边界、家庭激活、扫码状态机、恢复、管理者隔离、密文备份与撤销均通过。测试没有使用家庭真实内容，staging 已停止。
  - 生产本机／公网健康版本与镜像源码一致，家庭已初始化，六类匿名请求 401。原成员 1、设备 1、家庭 1、设置 1、备份清单 0 行逐行核对通过；仅设置中的模型字段更新为 Astra，暂停与额度保持。私有部署及回滚证据目录：`/opt/anan-ai/deployments/20260924-gpt6-astra/`。
  - 之前待部署的 `277d6c9` 转写结果完整性与 `8f7621f` 远端用量刷新修复已随本次服务端部署生效。安装包仍为上条 1.1.2；手机端 AI 任务缓存标识的更新在 main，未另出原生包。
