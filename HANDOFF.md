...
> 最后更新：2026-09-23。**1.0.4（构建号 77）发版中**：主人 2026-09-23 说「好的，发包吧」。发版提交在 main（`mobile/app.json` 1.0.4／构建号 77、CHANGELOG 顶节「1.0.4 — 首页整理」、README、本文；门禁全绿后提交），轻量标签 `v1.0.4` 指向它（发版提交 `703fe075a944e926995a507f7b9767543ab5de62`），标签推送触发 [run 35858197409](https://github.com/YePiXpert/family-time-capsule/actions/runs/35858197409)（12:04 UTC 起跑，结果待查）。内容是首页整理（`a973c21`／`864124e`／`510d835`）加出包流水线提速；服务端不动，生产仍是 1.0.3 的 `8be3a46`，不用部署。标签出包结果、Release 资产与校验和待 run 完成后补进第一节第一条；**在那之前不要把 1.0.4 当作已交付**，可安装的仍是 1.0.3。出包流水线第二轮提速已收尾：最后一次验证 [run 35855743789](https://github.com/YePiXpert/family-time-capsule/actions/runs/35855743789) 一次全绿，全程 14.7 分钟（基线 29.9；第一节第二条）。
...
- **1.0.4「首页整理」发版（构建号 77；2026-09-23）**：主人看过第二轮提速的验证结果后说「好的，发包吧」。发版提交改了四处：
  - `mobile/app.json` 的 1.0.4／77 三行版本更新。
  - CHANGELOG 顶节改名「1.0.4 — 首页整理（构建号 77）」，去掉「未打包」说明，补一条出包流水线提速。GitHub Release 的说明取这一节。
  - README 当前版本与本文。

  门禁全绿：mobile 49 个文件 729 项测试、server 183 项、typecheck／lint、边界脚本、脚本单测 35 项。轻量标签 `v1.0.4` 指向发版提交 `703fe075a944e926995a507f7b9767543ab5de62`，标签推送触发 [run 35858197409](https://github.com/YePiXpert/family-time-capsule/actions/runs/35858197409)（12:04 UTC 起跑）。**结果待查，不轮询**，主人通知后查一次：
    - `build-source.json` 的 gitSha 是 `703fe07…`，版本 1.0.4，构建号 77。
    - Release v1.0.4 上有 APK、未签名 IPA、`build-source.json`、`sha256sums.txt`，下载后重算的校验和一致。
    - 顺带看安卓 ccache 命中与全程时长。

  服务端不动，生产仍是 `8be3a46`。出包结果、Release 资产与校验和待 run 完成后补，**尚未交付**。
...
第二步·1.0.0（Build 73–76）**已交付**（第一节第一条：提交 16300f2、标签 v1.0.0、run 35607118819 四作业全绿、GitHub Release 上有 APK／IPA／校验和，不过期）。
   ...
   服务端生产已是 **8be3a46（1.0.3 减法：只剩 /ai/write 与五个 writingMode）**；部署验证与回滚材料在第一节，接下来做真机验收。
   ...
   **1.0.4「首页整理」（构建号 77）发版中**（第一节第一条）：只改手机端，服务端不动；标签 v1.0.4 的出包结果与 Release 校验和核对之前不算交付。
