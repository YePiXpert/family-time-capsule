...
> 最后更新：2026-09-24。**1.0.8（构建号 81）发版中，服务端一起部署**：主人说「发包吧，服务端也一起部署」。发版提交在 main（`mobile/app.json` 1.0.8／构建号 81、CHANGELOG 顶节「1.0.8」、README、本文；门禁全绿后提交），轻量标签 `v1.0.8` 指向它，标签推送触发出包；结果、Release 与校验和核对后补进第一节，**在那之前不要把 1.0.8 当作已交付**，可安装的仍是 1.0.7。服务端从同一发版提交构建镜像，先在 3141 staging 验证再切生产，结果见第一节。
