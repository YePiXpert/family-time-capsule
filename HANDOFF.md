...
- **出包流水线第二轮提速（2026-09-23，`ec60d63`／`8f5c03f`／`a7ea727`／`6f1310a`；冷编、热缓存、`6f1310a` 都已验证）**：主人要求继续压 CI 时间。拆 run 35729090883（`52b5f76`，29.9 分钟）：最长路径是 quality 1.9 分钟 → iOS 模拟器构建 16.5（其中 xcodebuild 14.1，几乎全花在 Reanimated 119／SVG 94／Screens 88／Codegen 42／Worklets 38 等约 450 个 C++／Objective-C++ 编译单元上；React Native 核心已是预编译）→ 两组 iOS 验证 10.8／11.2（模拟器冷启动 1.5～2.5 分钟，启动完成到 XCUITest 开跑 2.2～4.5 分钟，回归主用例 318～353 秒，启动冒烟首个场景 3.3 分钟）；Android 16.9（Gradle 8.2 + 模拟器冒烟 7.2）与 IPA 15.0 不在最长路径上。改法：
  ...
  - 下一步可选（待主人定）：①再派一次看安卓热缓存的效果（不改代码，175 个可缓存编译应当命中）；②安卓 ccache 加 `sloppiness = pch_defines,time_macros,include_file_mtime,include_file_ctime`，让 Reanimated 那部分预编译头编译也进缓存（Worklets／Expo 的预编译头每次内容不同，只会不命中，不会出错）；③安卓冒烟脚本本身 6.1 分钟，是下一块能压的地方。
