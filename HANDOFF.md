...
> 最后更新：2026-09-24。**1.0.8（构建号 81）已交付；服务端 `e200eb5` 已部署生产**：主人说「发包吧，服务端也一起部署」。发版提交 `e200eb5cc329dba1062e603a0269134c9340b909`，轻量标签 `v1.0.8`，[run 35967398609](https://github.com/YePiXpert/family-time-capsule/actions/runs/35967398609) 八个作业一次全绿，[GitHub Release v1.0.8](https://github.com/YePiXpert/family-time-capsule/releases/tag/v1.0.8) 双端包、源码记录、校验和与三份验证证据已下载核对（第一节第一条）。服务端同一提交 07:05 UTC 切到生产（第一节第二条）。以上都只是模拟器与 CI 的结果，**两台真机验收仍待完成**（清单见第一节第一条）。
