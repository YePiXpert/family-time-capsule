...
> 最后更新：2026-09-23。**main 上有一批未打包的改动：首页一屏放下与编辑页细节**（`7ab85f7`／`24068f5`，CHANGELOG 顶节「未打包 — 首页一屏放下与编辑页细节」；门禁全绿）。本机没有模拟器，界面只能看出包截图：验证出包 [run 35868419219](https://github.com/YePiXpert/family-time-capsule/actions/runs/35868419219)（源码 `6c6e841`，13:38 UTC 派出）**结果待查，不轮询**，通知后查一次并看截图（第一节第一条）。可安装的是 1.0.4（下一条）。
...
- **首页一屏放下与编辑页细节（2026-09-23，`7ab85f7`／`24068f5`，未打包）**：主人看了真机截图提两点——编辑页「更多：标题、地点、人物」与「放弃这份草稿」不合群、展开后更怪；首页上下滑动怪怪的，内容不多就让小地方左右切、用小图标，首屏固定。改法见 CHANGELOG 顶节与 DESIGN.md「书架」「编辑」两条，要点：
  ...
  - 门禁全绿：mobile 50 个文件 736 项测试、server 183 项、typecheck／lint、边界脚本、脚本单测 38 项。验证出包 [run 35868419219](https://github.com/YePiXpert/family-time-capsule/actions/runs/35868419219)（源码 `6c6e841667115a62963a5efa79fbae182292a07c`）结果待查，不轮询；通知后查一次，看 iOS 回归的 home／ai-entry／editor-details／editor-keyboard／record-bottom-bar、iOS 启动冒烟的 local-mode-dark／local-mode-large-text（更大文字时首页是否退回可滑）与安卓冒烟的 home-recent／home-320／home-390／editor 截图，再决定要不要调。
