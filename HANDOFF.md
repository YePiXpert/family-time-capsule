...
> 最后更新：2026-09-23。**main 上有一批未打包的改动：首页一屏放下与编辑页细节**（`7ab85f7`／`24068f5`／`dec20b5`，CHANGELOG 顶节「未打包 — 首页一屏放下与编辑页细节」；门禁全绿）。第一次验证出包 [run 35868419219](https://github.com/YePiXpert/family-time-capsule/actions/runs/35868419219) 的 iOS 回归红：有照片的「最近」卡把首页撑出了屏幕，`dec20b5` 已修（第一节第一条）；复验 [run 35870739215](https://github.com/YePiXpert/family-time-capsule/actions/runs/35870739215)（源码 `dec20b5`，13:58 UTC 派出）**结果待查，不轮询**，通知后查一次并看截图。可安装的是 1.0.4（下一条）。
