...
- **1.0.4「首页整理」发版（构建号 77；2026-09-23）**：主人看过第二轮提速的验证结果后说「好的，发包吧」。发版提交改了四处：
  ...
  门禁全绿：mobile 49 个文件 729 项测试、server 183 项、typecheck／lint、边界脚本、脚本单测 35 项。轻量标签 `v1.0.4` 指向发版提交，标签推送触发 `mobile-build.yml`。服务端不动，生产仍是 `8be3a46`。出包结果、Release 资产与校验和待 run 完成后补，**尚未交付**。
