...
> 最后更新：2026-09-24（1.1.0 发版时）。**1.1.0（构建号 82）发版中，服务端同日部署**：主人说「发包吧，服务端也一起部署」。发版提交在 main（`mobile/app.json` 1.1.0／构建号 82、CHANGELOG 顶节「1.1.0」、README、本文；门禁全绿后提交），轻量标签 `v1.1.0` 指向它，标签推送触发出包；结果、Release 与校验和核对后补进第一节，**在那之前不要把 1.1.0 当作已交付**。服务端从同一发版提交构建镜像，先 staging 再切生产，结果见第一节。
