...
- **1.1.2 安装包（构建号 84，2026-09-24 已触发，尚未交付）**：主人在代码审查后说「发包」。发布源码为 main 上的 `7e4a8a0788ada4b8daa4552db49d6f308f598a81`，轻量标签 `v1.1.2` 指向该提交并触发 [run 36015813965](https://github.com/YePiXpert/family-time-capsule/actions/runs/36015813965)；2026-09-24 14:51 UTC 唯一一次初始查询为 `queued`，结论为空。触发前核对过该完整 SHA 没有可复用原生构建，未另外手动 dispatch。修复范围见 CHANGELOG「1.1.2」，原有 11 个文件的未提交修改不进入本版；家庭初始化的分步界面仍在这些未提交修改里，本版仅补齐底层接口并保留原兼容调用。
  ...
  - 等待规则（主人 2026-09-24 更正）：只有 Fable 默认不轮询；其他所有模型（包括未知模型）每 30–60 秒轮询到完成，再验收交付。Fable 的委托与无法委托时的处理见 AGENTS.md。全部必要作业完成后再验证 APK／未签名设备 IPA、来源与 1.1.2／双端 84 的版本元数据、回归报告和 SHA-256，更新 README／本文并交付。在验证之前继续保留 1.1.1 为已交付版本。
  - 服务端 `277d6c9`（转写完整性）与 `8f7621f`（远端用量缓存）尚未部署；本次只出手机安装包，手机同步与取消修复不依赖这两项服务端部署。
  - 续接验收：按上述等待规则跟踪 run 至完成，失败确认后才读失败日志。必须通过 Package source、React Native quality、Android APK、iOS unsigned IPA、iOS simulator build、iOS startup verification、iOS regression verification 与 GitHub Release。下载 Release 的 APK、未签名设备 IPA、build-source.json、sha256sums.txt，核对完整 SHA、版本 1.1.2、Android 84／iOS 84 与校验和，并核对安卓回归、iOS 启动和 iOS 完整回归三份报告的来源和成功字段；之后再将 README／本文改为已交付并提供安装包链接。
