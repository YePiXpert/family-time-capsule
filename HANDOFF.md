...
> 最后更新：2026-09-24。**1.1.2（构建号 84）准备出包，尚未交付**（第一节第一条）；已交付版本仍为 1.1.1（构建号 83）。本轮未部署服务端，上次生产部署记录为 `bc948a1`。
>
...
- **1.1.2 安装包（构建号 84，2026-09-24 准备出包，尚未交付）**：主人在代码审查后说「发包」。从 main 的 `8f7621f` 加本次版本及文档提交构建，轻量标签 `v1.1.2` 将触发原生工作流；构建源码完整 SHA 与运行链接在触发后补齐。修复范围见 CHANGELOG「1.1.2」，原有 11 个文件的未提交修改不进入本版；家庭初始化的分步界面仍在这些未提交修改里，本版仅补齐底层接口并保留原兼容调用。
