...
- **出包流水线第二轮提速（2026-09-23，`ec60d63`／`8f5c03f`／`a7ea727`／`6f1310a`；冷编、热缓存都已测，`6f1310a` 的验证待查）**：主人要求继续压 CI 时间。拆 run 35729090883（`52b5f76`，29.9 分钟）：最长路径是 quality 1.9 分钟 → iOS 模拟器构建 16.5（其中 xcodebuild 14.1，几乎全花在 Reanimated 119／SVG 94／Screens 88／Codegen 42／Worklets 38 等约 450 个 C++／Objective-C++ 编译单元上；React Native 核心已是预编译）→ 两组 iOS 验证 10.8／11.2（模拟器冷启动 1.5～2.5 分钟，启动完成到 XCUITest 开跑 2.2～4.5 分钟，回归主用例 318～353 秒，启动冒烟首个场景 3.3 分钟）；Android 16.9（Gradle 8.2 + 模拟器冒烟 7.2）与 IPA 15.0 不在最长路径上。改法：
  ...
  - 主人 2026-09-23 说「一起做吧」，`6f1310a` 把下面几件一起做了（门禁全绿：mobile 729 项测试、typecheck、lint，server 183 项与 typecheck，脚本单测 22 项）：
    ...
    - 验证 [run 35832730488](https://github.com/YePiXpert/family-time-capsule/actions/runs/35832730488)（workflow_dispatch，源码 `6f1310a`，07:38 UTC 派出）**结果待查**，不轮询；主人告知完成后查一次：两个验证作业「Wait for the simulator build」各等了多久、回归有没有触发 `attempt-1/`、安卓 ccache 统计与缓存键是否存下、全程时长。
