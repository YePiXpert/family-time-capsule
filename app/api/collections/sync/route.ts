...
import { asRecord, mobileJson, readMobileJson, mobileRequestError } from "@/lib/mobile/http";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (Object.hasOwn(body, "familyId")) throw new CollectionError("family_id_not_accepted");
    return mobileJson(syncCollection(auth.context, body));
  } catch (error) {
    return error instanceof CollectionError ? mobileJson({ error: error.code }, { status: error.status }) : mobileRequestError(error);
  }
}
