...
import { submitDraftForReview } from "@/lib/drafts/service";
...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  ...
  try { const body = asRecord(await readMobileJson(request)); return mobileJson(submitDraftForReview(auth.context, (await route.params).id, body.expectedRevision as number)); } catch (error) { return draftResponseError(error); }
}
