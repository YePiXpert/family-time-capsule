...
export function SidebarNavigation({ familyName, inboxCount, userName, role, capabilities, simpleMode = false, displayMode = "standard" }: { familyName: string; inboxCount: number; userName: string; role: FamilyRole; capabilities: readonly FamilyCapability[]; simpleMode?: boolean; displayMode?: DisplayMode }) {
            ...
            return (
              <li key={item.href}>
                <Link href={item.href} aria-current={active ? "page" : undefined} className={`sidebar-nav-item ${item.emphasis ? "sidebar-nav-item-emphasis" : ""} ${active ? "is-active" : ""}`}>
                  ...
                  <span>{simpleMode && item.href === "/capture" ? "说几句" : item.label}</span>
