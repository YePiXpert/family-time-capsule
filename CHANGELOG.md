...
## 0.0.1 — 早期发行版（2026-09-11，原生构建 35）

- 按维护者要求将产品版本重新编号为 0.0.1，发布序号延续至 18；旧 tag、记录、迁移和原生构建号保留。
- 汇总移动端运行时拆分、本机启动恢复、串行弹窗、系统分享与同步协调改进。
- 首页和公共组件采用奶油纸色、陶土主操作、内容优先的记录卡片，支持深色和大字号。
- 部署先备份、复制新卷、隔离验证，再成对切换镜像与卷；快照回滚分阶段执行。
- 安装包、服务镜像、运维工具和校验清单见 [0.0.1 发布说明](docs/releases/0.0.1.md)。
...
## 1.0.0-dev.1 — 正式 1.0 开发中（未发布 stable）

- M3-A：记忆年龄锚点可空；无孩子家庭首次建立、祖辈口述、夫妻旅行可以正常创建与编辑。
  0051 SQLite 表重建保留旧事件及关系；启动前一致快照、事务内外键检查，失败关闭。
- 参与人物与年龄锚点独立；首页、时间线、详情、移动同步、作品及导出按事件自己的参考人物
  计算年龄，无锚点只显示日期，成年人按岁显示；Web/原生旧缓存和归档恢复兼容。
- 核对 M2/M6/M7-a 实现，修正 owner、儿童监护、访客阅读及 FIND-3/4 过期文档。
  自然语言评测现有脚本仅为关键词基线与理想计划上限，真实模型结论仍待验证。
- 版本仍为 1.0.0-dev.1；外部门禁未解除，不创建 v1.0.0 tag。
...
- 首页与导航：小字对比度提升至 WCAG AA；底部导航补上「家庭投递箱」高亮，移动端与
  桌面端激活规则统一；桌面侧边栏新增「家庭投递箱」「批量导入」入口；只读成员不再被
  空态引导到无权限的记录页；每周回顾卡去掉伪按钮；确认弹窗遮罩适配暗色主题。
- 时间线与记忆详情：「移到回收站」增加确认弹窗与进行中反馈；日历封面不再回退加载
  全尺寸原图，无缩略图时显示占位；时间轴与阅读器缩略图改为懒加载并固定宽高比，消除
  布局跳动；阅读器按素材自身拍摄时间标注日期；筛选后保留相册上下文，空结果不再显示
  无效的归入面板；月份优先于年份时下拉框正确回显；详情页相关记忆改为并行加载。
- 记录与上传：访客贡献提交成功后清空表单并提供「再提交一份」入口，杜绝重复投递；
  选择文件不再覆盖已录制的语音备忘录，已选文件可见、可逐项移除，超出上限有提示；
  上传增加超时保护，断网假死变为可重试的错误；原始错误码替换为可读中文提示；
  「先收进来」成功后给出「去收件箱整理」链接并清空已存内容；批量导入支持拖拽加入，
  有未完成文件时离开页面会提示。
- 书籍编辑器：离开页面时自动补存未保存的修改；自动保存暂停后给出明确提示；删除
  内容块需要确认；空章节等出版警告集中展示并按严重度分级；排版排队状态显示进度；
  内容块支持拖拽排序与「移动到章节」；桌面宽屏下编辑器右侧并排实时预览。
- 移动端：书籍封面选择改为单行入口的展开式选择器，不再平铺全部候选按钮。
- 根/mobile package 与 lock 均为 `1.2.0-alpha.4`；原生展示版本 `1.2.0`，构建号递增为
  `10`。本版本继续为 prerelease，真实设备验收尚未完成。

## 1.2.0-alpha.3 — 首页快捷入口闪退修复（2026-09-05）

- 修复首页“文字 / 拍照 / 录音 / 导入”携带参数跳转时，BottomTabBar 调用不存在的
  `queryString.stringify` 导致原生应用退出的问题。保留修复解码漏洞的 query-string 9.5.1，
  安装后将 React Navigation 的源码与发布 JS 统一改为该版本的默认导入。
- 新增五项使用真实已安装导航库的参数生成、清除与解析回归测试；修复前五项均失败，
  修复后通过。安装补丁校验导航库版本和原始导入，依赖升级需要重新审核兼容性。
- 修复 iOS 相机不可用时 UIImagePickerController 抛出原生异常导致拍照入口退出的问题；
  在设置相机来源与设备前检查可用性，失败时提示从相册导入并保留记录页面。
- 增加 iOS Release 模拟器首页入口测试，采集截图及系统崩溃日志；测试包使用 Xcode
  模拟器签名，设备 Release 的 unsigned IPA 流程保留。
- 原生构建号递增到 `9`，展示版本仍为 `1.2.0`；包含 alpha.2 的全部修复。

## 1.2.0-alpha.2 — 录音、媒体与本机隐私修复（2026-09-05）

- 录音器延迟到权限与音频会话准备完成后创建，失败后清理并允许重试，修复记录入口崩溃。
- 媒体端点正确处理后缀 Range（例如 `bytes=-32` 返回最后 32 字节）、开放范围与超大数字；
  无法满足的范围返回 416，格式错误或不支持的范围按完整响应处理。
- 原生记忆详情缓存按服务器、会话摘要、账号和家庭隔离；切换连接时丢弃旧异步结果，
  401/403/404 清除对应缓存。升级移除无法确认归属的旧详情缓存，本机原件和待同步记录保留；
  升级或重新登录后，需联网打开详情一次以重新建立缓存。
- “清空本机数据”先中止并等待阅读下载、同步和导入结束，再清理所有连接范围的下载、
  绑定及孤立文件；清理期间阻止新任务，清理失败会提示重试。
- 根/mobile package 与 lock 均为 `1.2.0-alpha.2`；原生展示版本 `1.2.0`，构建号递增为 `8`。
  APK 为测试签名包，IPA 未签名；本版本继续为 prerelease，真实设备验收尚未完成。
