...
type Props = OrganizerTarget & { label?: string; onSaved?: () => void };
export function OrganizerPanel(props: Props) {
  ...
  if (!["admin", "editor"].includes(viewer?.role ?? "")) return props.kind === "asset" ? <TranscriptEditor assetId={props.id} label={props.label ?? "录音或视频"} /> : null;
  return <Panel key={JSON.stringify([scope, props.kind, props.id])} {...props} scope={scope} />;
}
function Panel({ kind, id, label, onSaved, scope }: Props & { scope: string }) {
  ...
  return <View>
    <View style={s.card}>
      <Pressable onPress={() => setOpened(!opened)} style={s.secondaryButton}><Text style={s.secondaryText}>{kind === "asset" ? "转成文字" : "AI 帮我起名"}</Text></Pressable>
      {opened ? <>
        ...
        {review ? <>
          ...
          <Pressable disabled={disabled || active || !capability?.available || !capability.consented} onPress={() => void mutate(kind === "asset" ? "transcribe" : "name")} style={[s.primaryButton, disabled && s.disabled]}><Text style={s.primaryText}>{kind === "asset" ? "开始转成文字" : "生成标题建议"}</Text></Pressable>
    ...
    {kind === "asset" ? <TranscriptEditor assetId={id} label={label ?? "录音或视频"} onSaved={onSaved} refreshVersion={refreshVersion} /> : <NameEditor kind={kind} id={id} onSaved={onSaved} refreshVersion={refreshVersion} />}
  </View>;
}
