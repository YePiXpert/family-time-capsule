...
function EditorButton({ label, onPress, pending }: { label: string; onPress: () => void; pending: boolean }) {
  return <Pressable accessibilityRole="button" disabled={pending} onPress={onPress} style={sharedStyles.secondaryButton}><Text style={sharedStyles.secondaryText}>{label}</Text></Pressable>;
}
...
export function MemoryEditor({ memory, onSaved }: { memory: MobileMemory; onSaved: () => Promise<void> }) {
  ...
  return <View style={sharedStyles.card} accessibilityLabel="编辑与分享">
    ...
    {content ? <>
      <TextInput editable={!pending} accessibilityLabel="记忆标题" value={content.title} maxLength={100} onChangeText={title => setContent(v => v && ({ ...v, title }))} style={sharedStyles.input} />
      <TextInput editable={!pending} accessibilityLabel="记忆正文" value={content.bodyText} multiline maxLength={100000} onChangeText={bodyText => setContent(v => v && ({ ...v, bodyText }))} style={sharedStyles.input} />
      ...
      <TextInput editable={!pending} accessibilityLabel="记忆地点" value={content.location} maxLength={200} onChangeText={location => setContent(v => v && ({ ...v, location }))} style={sharedStyles.input} />
      <Text style={sharedStyles.body}>参与人物</Text>{people.map(person => <Pressable disabled={pending} key={person.id} accessibilityRole="checkbox" accessibilityLabel={person.displayName} accessibilityState={{ checked: content.participants.includes(person.id) }} onPress={() => setContent(v => v && ({ ...v, participants: v.participants.includes(person.id) ? v.participants.filter(id => id !== person.id) : [...v.participants, person.id] }))} style={sharedStyles.secondaryButton}><Text style={sharedStyles.secondaryText}>{content.participants.includes(person.id) ? "✓ " : ""}{person.displayName}</Text></Pressable>)}
    </> : null}
    {sharing ? <>
      <Text style={sharedStyles.cardTitle}>谁可以阅读这件事</Text>
      {([ ["private", memory.isAuthor ? "仅自己" : "仅作者"], ["members", "指定家人"], ["family", "全家可见"] ] as const).map(([value, label]) => <Pressable disabled={pending} key={value} accessibilityRole="radio" accessibilityState={{ selected: sharing.visibility === value }} onPress={() => setSharing(v => v && (v.visibility === value ? v : { ...v, visibility: value, readers: [] }))} style={sharedStyles.secondaryButton}><Text style={sharedStyles.secondaryText}>{label}</Text></Pressable>)}
      {sharing.visibility === "members" ? <>{readerError ? <Text>{readerError}</Text> : null}{readers.map(reader => <Pressable disabled={pending} key={reader.id} accessibilityRole="checkbox" accessibilityLabel={reader.name} accessibilityState={{ checked: sharing.readers.includes(reader.id) }} onPress={() => setSharing(v => v && ({ ...v, readers: v.readers.includes(reader.id) ? v.readers.filter(id => id !== reader.id) : [...v.readers, reader.id] }))} style={sharedStyles.secondaryButton}><Text style={sharedStyles.secondaryText}>{sharing.readers.includes(reader.id) ? "✓ " : ""}{reader.name}</Text></Pressable>)}{sharing.readers.filter(id => !readers.some(reader => reader.id === id)).map(id => <View key={id}><EditorButton pending={pending} label="取消已不可用的成员" onPress={() => setSharing(v => v && ({ ...v, readers: v.readers.filter(value => value !== id) }))} /></View>)}</> : null}
      <Text style={sharedStyles.body}>保存后才改变读者。移除读者会收回这件事的在线入口；其他独立分享仍有效，已导出的文件无法远程收回。家人讲述也遵循各自的范围。</Text>
    </> : null}
