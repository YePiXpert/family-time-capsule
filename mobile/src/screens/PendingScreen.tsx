...
export function PendingScreen() {
  ...
  return <ScrollView style={s.screen} contentContainerStyle={s.content}>
    ...
    {drafts?.local.map(d => <Pressable key={d.id} accessibilityRole="button" onPress={() => navigation.navigate("Capture", { scope, target: { kind: "local", draftId: d.id, editSaved: d.status === "queued" } })} style={s.card}><Text style={s.cardTitle}>{d.content.title || d.content.text.slice(0, 40) || "照片与声音"}</Text><Text style={s.body}>{d.status === "queued" ? "待同步" : "继续记录"}</Text></Pressable>)}
    {drafts?.remote.map(d => <Pressable key={d.id} accessibilityRole="button" onPress={() => navigation.navigate("Capture", { scope, target: { kind: "serverDraft", draftId: d.id } })} style={s.card}><Text style={s.cardTitle}>{d.title || d.text.slice(0, 40) || "照片与声音"}</Text><Text style={s.body}>继续记录</Text></Pressable>)}
