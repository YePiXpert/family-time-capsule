...
function Detail({ id }: { id: string }) {
  ...
  const remove = () => void confirm({ title: "永久删除原件", message: "删除这份原件及预览，并移除未确认整理项中的引用。仍被草稿、记忆、相册或作品使用时会拒绝删除。已有下载和历史备份可能仍保留副本。", confirmLabel: "确认永久删除", destructive: true })
    .then(confirmed => { if (!confirmed || !credentials) return; void requestMobileJson(credentials, `/api/mobile/v1/assets/${id}`, { method: "DELETE", body: JSON.stringify({ confirmed: true }) }).then(() => navigation.replace("AssetLibrary")).catch(e => setMessage(e.message)); });
