...
export function SearchScreen({ navigation }: Props) {
  ...
  return <View style={sharedStyles.screen}>
    ...
    <FlatList contentContainerStyle={items.length === 0 ? { flexGrow: 1 } : styles.list} data={items} keyExtractor={(item) => `${item.type}:${item.id}`} ListEmptyComponent={!loading ? <View style={sharedStyles.empty}><Text style={sharedStyles.emptyTitle}>{query ? "没有找到相关内容" : "找回一段家庭记忆"}</Text><Text style={sharedStyles.emptyText}>输入人物、地点、标题或讲述中的字词。</Text></View> : null} ListFooterComponent={loading ? <ActivityIndicator color={colors.coral} /> : cursor ? <Pressable onPress={() => void search(cursor)} style={sharedStyles.secondaryButton}><Text style={sharedStyles.secondaryText}>加载更多</Text></Pressable> : null} renderItem={({ item }) => <Pressable disabled={!item.eventId} onPress={() => item.eventId ? navigation.navigate("Memory", { id: item.eventId }) : undefined} style={({ pressed }) => [sharedStyles.card, pressed && sharedStyles.pressed]}><Text style={styles.kind}>{item.type === "memory" ? "记忆" : item.type === "contribution" ? "家人讲述" : item.type === "story" ? "故事" : "档案内容"}</Text><Text style={sharedStyles.cardTitle}>{item.title}</Text><Text numberOfLines={3} style={sharedStyles.body}>{item.snippet}</Text></Pressable>} />
  </View>;
}
