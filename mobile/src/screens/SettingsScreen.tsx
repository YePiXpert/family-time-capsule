...
export function SettingsScreen() {
  ...
  return <ScrollView contentContainerStyle={s.content} style={s.screen}>
        ...
        {failed.map((item) => (
          <View key={item.id} style={styles.failedItem}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.warningText}>{item.lastError} · 已尝试 {item.attemptCount} 次</Text>
            </View>
    ...
    <View style={s.card}>
      <Text style={s.body}>把尚未同步的本机记录（文字全文、原件与校验清单）导出为一个 ZIP，通过系统分享保存到 App 之外。它只包含你自己的本机资料，不含任何登录凭据，也不是完整家庭备份。</Text>
      <Pressable disabled={rescueBusy} onPress={() => void exportRescue()} style={s.secondaryButton}><Text style={s.secondaryText}>导出本机救援包</Text></Pressable>
      <Pressable disabled={rescueBusy} onPress={() => void importRescue()} style={s.secondaryButton}><Text style={s.secondaryText}>从救援包恢复</Text></Pressable>
    </View>
...
function createStyles(palette: JournalPalette) {
  ...
  row: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 16, borderBottomColor: palette.line, borderBottomWidth: StyleSheet.hairlineWidth },
  label: { color: palette.muted, fontSize: 13 },
  value: { flex: 1, color: palette.ink, fontSize: 13, fontWeight: "700", textAlign: "right" },
  textButton: { minHeight: 48, alignItems: "center", justifyContent: "center" },
  danger: { color: palette.error, fontSize: 14, fontWeight: "800" },
  warningTitle: { color: palette.warning, fontSize: 16, fontWeight: "800" },
  item: { color: palette.ink, fontSize: 13, fontWeight: "700", marginTop: 8 },
  failedItem: { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 10 },
  smallAction: { borderColor: palette.sage, borderRadius: 10, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 8, minHeight: 48, justifyContent: "center" },
  smallActionDanger: { borderColor: palette.dangerLine, borderRadius: 10, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 8, minHeight: 48, justifyContent: "center" },
  smallActionText: { color: palette.coralDark, fontSize: 12, fontWeight: "800" },
  failedNote: { color: palette.muted, fontSize: 12, lineHeight: 17, marginTop: 8 },
  clear: { minHeight: 48, alignItems: "center", justifyContent: "center", borderColor: palette.dangerLine, borderRadius: 13, borderWidth: 1, marginTop: 8 },
  });
}
