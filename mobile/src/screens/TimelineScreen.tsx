...
export function TimelineScreen() {
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => <TimelineCard item={item} timeZone={item.source === "server" ? family?.timezone : undefined} onPress={() => item.source === "server" ? navigation.navigate("Memory", { id: item.id }) : Alert.alert("本机记录", item.syncState === "synced" ? "这份原件已送达收件箱，整理确认后会成为正式记忆。" : "这份原件仍在本机，联网后会继续补传。") } />}
