...
const mocks = vi.hoisted(() => ({ credentials: null as Credentials | null, userId: null as string | null, family: null as { id: string } | null, permission: 0, exists: true, popTo: vi.fn() }));
...
vi.mock("../src/state/AppContext", () => ({ useAppData: () => ({ ...mocks, viewer: mocks.userId ? { id: mocks.userId, canCapture: true } : null, people: [] }), useSyncStatus: () => ({ syncing: false }) }));
...
const render = (scope = "local") => createElement(SavedMemoryScreen, { route: { key: "reading", name: "SavedMemory", params: { draftId: "saved-story", scope } }, navigation: { popTo: mocks.popTo } } as never);
...
it("reads a saved offline story after restart and returns to the existing composer for a supplement", async () => {
  ...
  await act(() => tree!.root.findByProps({ accessibilityLabel: "补记" }).props.onPress());
  expect(mocks.popTo).toHaveBeenCalledWith("MainTabs", { screen: "Capture", params: { localDraftId: "saved-story", editSaved: true } });
});
