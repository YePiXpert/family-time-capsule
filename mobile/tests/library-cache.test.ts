...
const { StoryDetailScreen } = await import("../src/screens/LibraryScreens");
const store = await import("../src/storage/database");
const { ApiError } = await import("../src/api/client");
const detail: MobileLibraryDetail = { id: "story", title: "私密来源故事", paragraphs: [{ id: "p", text: "撤权后不可读的段落", sources: [] }] };
(globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
let tree: ReactTestRenderer | undefined;
const render = () => createElement(StoryDetailScreen,{ route: { params: { id: "story" } }, navigation: {} } as never);
...
beforeEach(async () => {
  ...
  await store.initializeLocalStore(); await store.clearLocalArchive(); await store.cacheMobileLibraryDetail("stories",detail);
});
afterEach(async () => { if (tree) await act(async () => tree!.unmount()); tree = undefined; });
it.each([401,403,404])("HTTP %i removes the cached story instead of falling back to revoked text", async status => {
  ...
  expect(await store.getCachedMobileLibraryDetail("stories","story")).toBeNull();
});
it("a transient transport failure keeps the authorized offline story", async () => {
  mocks.fetch.mockRejectedValue(new ApiError("offline",0));
  await act(async () => { tree = create(render()); });
  expect(output()).toContain("撤权后不可读的段落");
});
it("a permission reset clears rendered story text and rejects the old pending response", async () => {
