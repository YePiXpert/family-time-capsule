...
在「我的 → AI 设置」输入邀请码并为设备命名。主人可管理成员、设备、模型和每日额度。

编辑照片记录时点「AI 按事情分组」或「帮这件事写记录」，预览并采用建议，再保存记录。GPT 与 DeepSeek 统一经 VPS 的 CPA 调用，API Key 不进入安装包。分组保留每张照片的拍摄信息，不将同一天的事情强制合并。
