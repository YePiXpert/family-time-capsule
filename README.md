...
**0.0.1 / 早期发行版 · 当前原生构建 42**

构建 42 统一记录、选材与相册体验：苔绿主题、全屏编辑、离线本机相册及明确授权后的家庭同步，见 [本轮改动与服务端升级说明](docs/releases/mobile-build-42.md)。APK 与未签名 IPA 由 [Native mobile packages](https://github.com/YePiXpert/family-time-capsule/actions/workflows/mobile-build.yml) 从同一 main 提交构建，具体结果与附件以该提交的 workflow 为准。固定发行版仍见 [v0.0.1 Release](https://github.com/YePiXpert/family-time-capsule/releases/tag/v0.0.1)（原生构建 35）。
...
- **成长**：按真实发生日期浏览、搜索和筛选；日期不确定时如实标注。
  原生首页可直接点选年份和月份，条数来自本机记录，保留真实月龄筛选。
