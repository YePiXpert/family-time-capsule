...
匿名接口（激活、配对申请与领取、恢复）按来源地址和全局两级限流，配对申请同一地址最多同时挂 3 条、全服务 20 条；恢复接口只把核对不过的尝试记进限流，对的恢复码不会被挡。来源地址默认是连接地址：经反代进来时所有人共用反代的地址，按地址的限流等于全家共用一份；配对名额这时不按地址分（没设 `TRUST_PROXY` 时，本机回环与私有网段的连接当作反代本身），只算全服务 20 条。要让它按真实客户端地址分开，在私有 env 里设 `TRUST_PROXY` 为容器看到的反代来源地址（Docker 端口转发有时会换成网桥网关地址，先在容器里用 `ss -tn` 核对），并确认反代覆盖或追加 `X-Forwarded-For`；只写反代自己，不要写 `true`、跳数或公网网段，写错服务拒绝启动。
...
- JSON 请求体上限 1 MiB，发布清单和回收为 4 MiB；除匿名接口外都先验令牌再读请求体。关停（`docker stop`）时在途请求有 8 秒答完，之后断开连接、关好数据库再退出。
